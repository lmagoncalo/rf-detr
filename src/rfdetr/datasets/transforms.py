...
class AlbumentationsWrapper:
    ...
    def __call__(
        ...
        # === Apply Transform ===
        if self._is_geometric and ("masks" in target or "keypoints" not in target) and "boxes" not in target:
            logger.warning(
                "AlbumentationsWrapper: geometric transform requested with 'masks' or 'keypoints' but without 'boxes'. "
                "Masks/Keypoints will not be geometrically transformed because bounding boxes are missing."
            )
